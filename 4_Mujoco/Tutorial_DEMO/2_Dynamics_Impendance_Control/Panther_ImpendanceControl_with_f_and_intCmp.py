...
# ---------- 阻抗控制参数 ----------
# KLM
K = np.array([50, 65, 110, 130, 110, 300])#刚度系数k
B = np.array([8, 8, 9.5, 9, 7, 8])#阻尼系数b
# MIT
# K = np.array([3.5, 6.5, 8.5, 2.5, 1.5, 0.80])#刚度系数k
# B = np.array([0.85, 1.05, 1.05, 0.35, 0.15, 0.13])#阻尼系数b
# 共享变量
q_des = np.array([0.0, 0.9, 0.9, 0.0, 0.0, 0.0])  # 期望目标位置
v_des = np.zeros(6)
int_error = np.zeros(6)  # 积分误差
Ki = np.array([2.6, 4.0, 4.3, 4.5, 3.5, 1.5])  # 积分增益，可以调
tau_int_max = np.array([0.15, 0.6, 0.52, 0.45, 0.08, 0.2]) * 4  # 静摩擦力矩的 3.5 倍限制
tau_int_min= np.array([0.03, 0.1, 0.52, 0.45, 0.08, 0.2])
...
def impedance_control_loop():
    ...
    while True:
        ...
        # 更新积分项
        error = q_des - q[:6]
        # 只有当系统没被扰动，误差较小快静止时才启动积分
        # 仅当系统已进入近似稳态时才积累积分误差
        if np.all(np.abs(error) < 0.8) :
        # 积分带泄露
            int_error = 0.99 * int_error + error * control_period
        else :
            int_error *= 0.95

        # 限幅积分量，避免过饱和（以力矩作为限制指标）
        tau_int = Ki * int_error
        tau_int = np.clip(tau_int, -tau_int_max, tau_int_max)
        print("tau_int:", tau_int, "error:", error)
        ...
        # with f and limit int compensate
        tau = M @ acc_des + C @ vel[:6] + G[:6] + f[:6] + tau_int
